...
export function buildOverlayReviewContext(tracks: OverlayTrack[]): Record<string, unknown> {
  return {
    ...
    rules: [
      ...
      "A boost is °C/min on RoR-error, not the BOOST kit. Rest does not auto-add an into-crack +boost. Drying / Maillard / after-crack zones only. Do not ask the generator to slam heat at first crack.",
